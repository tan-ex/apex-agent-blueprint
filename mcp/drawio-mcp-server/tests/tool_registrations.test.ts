 ...
 * and that registerTools wires all 27 tools to the MCP server.
 */
...
import {
  ...
  TOOL_DEFINITIONS,
  TOOL_NAMES,
} from "../src/tool_registrations.ts";
import {
...
describe("TOOL_NAMES", () => {
  it("should contain 27 tool name entries", () => {
    const entries = Object.entries(TOOL_NAMES);
    assertEquals(entries.length, 27);
  });
  ...
  it("should have all values in kebab-case format", () => {
    ...
    for (const [key, value] of Object.entries(TOOL_NAMES)) {
      assert(
        ...
        `${key} should be kebab-case, got "${value}"`,
      );
    }
  });
  ...
  it("should have UPPER_SNAKE_CASE keys", () => {
    ...
    for (const key of Object.keys(TOOL_NAMES)) {
      assert(
        upperSnakePattern.test(key),
        `key "${key}" should be UPPER_SNAKE_CASE`,
      );
    }
  });
});
...
describe("TOOL_DEFINITIONS", () => {
  it("should contain 27 tool definitions", () => {
    assertEquals(TOOL_DEFINITIONS.length, 27);
  });
  ...
  it("should have non-empty descriptions for all tools", () => {
    for (const def of TOOL_DEFINITIONS) {
      assert(
        ...
        `${def.name} description should be non-empty`,
      );
    }
  });
  ...
  it("should have inputSchema when hasArgs is true and not when false", () => {
    for (const def of TOOL_DEFINITIONS) {
      if (def.hasArgs) {
        assert(
          "inputSchema" in def,
          `${def.name} should have inputSchema when hasArgs is true`,
        );
      } else {
        assert(
          !("inputSchema" in def),
          `${def.name} should not have inputSchema when hasArgs is false`,
        );
      }
    }
  });
...
describe("registerTools", () => {
  ...
  function createMockToolHandler(): ReturnType<
    typeof createToolHandlerFactory
  > {
  ...
  it("should register exactly 27 tools", () => {
    ...
    assertEquals(registerSpy.calls.length, 27);
    registerSpy.restore();
  });
  ...
  it("should register each tool with a description", () => {
    ...
    for (const call of registerSpy.calls) {
      ...
      assert(
        ...
        `tool "${toolName}" description should be non-empty`,
      );
    }
    registerSpy.restore();
  });
