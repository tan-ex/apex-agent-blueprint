...
agent: agent
model: "GPT-5.4 mini"
description: "Stage everything except agent-output/ and infra/, conventional commit, push, then prompt to open or update a PR. CLI-only (git + gh)."
...
Stage all changes **except** anything under `agent-output/` or `infra/`,
write a conventional commit, push to the current branch, then ask whether
to open a new PR or update an existing one. Uses `git` and `gh` only — no
MCP tools.
...
git diff --stat HEAD -- . ':!agent-output' ':!infra'
```
...
Show a one-line note of any changes under `agent-output/` or `infra/` so the
user knows they were intentionally skipped:
...
```bash
git add -A -- . ':!agent-output' ':!infra'
git diff --cached --stat
```

### Step 3 — Compose conventional commit
...
…and propose a message in this shape:
...
Then call `vscode/askQuestions` with one question:

- header: `commit-message`
- question: `Use this commit message?`
- options:
  - `Yes, commit as-is` (recommended)
  - `Edit message` (freeform reply will replace it)

This is the **only** confirmation gate before the commit.

### Step 4 — Commit and push

```bash
git commit -m "<confirmed subject>" -m "<confirmed body>"
...
- Only one confirmation gate before commit (Step 3) and one before PR action
  (Step 5). No other round-trips.
