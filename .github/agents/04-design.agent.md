...
handoffs:
  - label: "▶ Generate Diagram"
    agent: 04-Design
    prompt: "Generate an Azure architecture diagram using the azure-diagrams skill (draw.io default). Produce `agent-output/{project}/03-des-diagram.drawio` with deterministic layout, enforced naming conventions, and quality score >= 9/10."
    send: false
  - label: "▶ Generate ADR"
    agent: 04-Design
...
- Always generate draw.io (`.drawio`) for architecture diagrams
